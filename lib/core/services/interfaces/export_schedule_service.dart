...
abstract interface class ExportScheduleService {
  ...
  /// 
  /// Если ниразу не запрашивалось разрешение на доступ к календарю, то оно запросится
  /// 
  /// Перед вызовом рекомендуется вызвать [requestCalendarPermission] и убедиться, что разрешение на использование календаря есть
  /// 
